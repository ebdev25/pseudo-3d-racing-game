                              ...
                              bool disableWrapping)
{
    ...
    for (int i = 0; i < 3; i++) {
        ...
        else
        {
            ...
            // --------------------------------------------------
            // We'll just place the entire layer at (offsetX, offsetY).
            // Then we do some simple clipping if part is off-screen.
...
static void renderTrafficLight(Game* game)
{
    ...
    // Use the segment you placed them on. Let’s say it’s near index 2
    // or you store a dedicated “startLineIndex”
    ...
    // We can pick a scale from seg->p1 (or interpolate p1/p2).
    // If we want them pinned in the same 2D spot, we can do a
    // "project" approach. But let's just do simpler:
...
void render(Game* game) {
    ...
    for (int n = 0; n < drawDistance; n++) {
        ...
        //SDL_Log("[DEBUG] Looped: %d, Current Index: %d, Base Index: %d, Position: %.2f, CameraZ: %.2f, TrackLength: %.2f",
        //looped, segment->index, baseSegment->index, position, cameraZ, game->road.trackLength);
    ...
    //SDL_Log("Highest Y: %f", game->highestRoadY);
                         ...
                         bool disableWrapping)
{
    ...
    // 3) Render the layers of the CURRENT state
    renderStateLayers(renderer, current, screenWidth, screenHeight, disableWrapping);

    // 4) If a transition is happening, also render the TARGET state's layers
...
void renderAIOpponents(Game* game, Segment* segment)
{
    ...
    for (int i = 0; i < ctrl->driverCount; i++) {
        ...
        // Render collision animation if active
        /*
        if (driver->collisionAnim.active && !driver->collisionAnim.finished) {
            const Sprite* currentFrame = driver->collisionAnim.frames[driver->collisionAnim.currentFrame];
            renderSprite(game, aiTex, currentFrame, spriteScale, spriteX, spriteY, -0.5, -1.0, seg->clip);
        }
        */
    }
}
